...
def test_low_pass_cutoff_formula() -> None:
    ...
    assert result == "1/(2*PI()*1000*1e-6)"

    # Test with cell references
    result = formula.build("A2", "B2")
    assert result == "1/(2*PI()*A2*B2)"
...
def test_high_pass_cutoff_formula() -> None:
    ...
    assert result == "1/(2*PI()*10000*100e-9)"
...
def test_band_pass_center_formula() -> None:
    ...
    assert result == "1/(2*PI()*SQRT(1e-3*1e-9))"

    # Test with cell references
    result = formula.build("A2", "B2")
    assert result == "1/(2*PI()*SQRT(A2*B2))"
...
def test_filter_attenuation_formula() -> None:
    ...
    assert result == "-20*2*LOG10(10000/1000)"

    # Test with cell references
    result = formula.build("A2", "B2", "C2")
    assert result == "-20*C2*LOG10(A2/B2)"
...
def test_low_pass_cutoff_typical_values() -> None:
    ...
    assert result == "1/(2*PI()*1000*0.000001)"
...
def test_high_pass_cutoff_typical_values() -> None:
    ...
    assert result == "1/(2*PI()*10000*1e-7)"
...
def test_band_pass_center_typical_values() -> None:
    ...
    assert result == "1/(2*PI()*SQRT(0.001*1e-9))"
...
def test_filter_attenuation_various_orders() -> None:
    ...
    assert result == "-20*1*LOG10(10000/1000)"
    ...
    assert result == "-20*4*LOG10(10000/1000)"
...
def test_formulas_with_scientific_notation() -> None:
    ...
    assert low_pass.build("1e3", "1e-9") == "1/(2*PI()*1e3*1e-9)"
    assert high_pass.build("1e4", "1e-12") == "1/(2*PI()*1e4*1e-12)"
    assert band_pass.build("1e-6", "1e-12") == "1/(2*PI()*SQRT(1e-6*1e-12))"
...
def test_filter_attenuation_below_cutoff() -> None:
    ...
    assert result == "-20*1*LOG10(100/1000)"
...
def test_filter_design_workflow() -> None:
    ...
    assert cutoff_formula == "1/(2*PI()*1000*1.59e-7)"

    # Calculate Q factor
    q_formula = q_factor.build("1000", "200")
    assert q_formula == "1000/200"

    # Calculate attenuation at 10kHz
    atten_formula = attenuation.build("10000", "1000", "2")
    assert atten_formula == "-20*2*LOG10(10000/1000)"
