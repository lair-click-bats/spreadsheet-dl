...
def test_plugin_initialization() -> None:
    ...
    assert plugin.get_formula("ENTROPY_CHANGE") == EntropyChangeFormula
    ...
    assert plugin.get_formula("IDEAL_GAS_LAW") == IdealGasLawFormula
    ...
    # Verify total count
    assert len(plugin.list_formulas()) == 20
...
def test_entropy_change_formula() -> None:
    """Test ENTROPY_CHANGE formula."""
    formula = EntropyChangeFormula()
    metadata = formula.metadata

    assert metadata.name == "ENTROPY_CHANGE"
    assert metadata.category == "thermodynamics"
    assert len(metadata.arguments) == 2
...
def test_ideal_gas_law_formula() -> None:
    """Test IDEAL_GAS_LAW formula."""
    formula = IdealGasLawFormula()
    metadata = formula.metadata

    assert metadata.name == "IDEAL_GAS_LAW"
    assert metadata.category == "thermodynamics"
    assert len(metadata.arguments) == 5
