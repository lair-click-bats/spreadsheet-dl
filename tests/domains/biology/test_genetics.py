...
def test_hardy_weinberg_formula() -> None:
    ...
    assert result == "0.6^2 + 2*0.6*0.4 + 0.4^2"
    assert "^2" in result
    assert "2*" in result

    # Test with cell references
    result = formula.build("A1", "B1")
    assert result == "A1^2 + 2*A1*B1 + B1^2"
...
def test_chi2_genetics_formula() -> None:
    ...
    assert result == "SUMPRODUCT((A1:A4 - B1:B4)^2 / B1:B4)"
...
def test_inbreeding_coefficient_formula() -> None:
    ...
    assert result == "1 - (0.25/0.5)"
    assert "1 -" in result

    # Test with cell references
    result = formula.build("A1", "B1")
    assert result == "1 - (A1/B1)"
...
def test_hardy_weinberg_with_different_frequencies() -> None:
    ...
    assert result == "0.5^2 + 2*0.5*0.5 + 0.5^2"

    # Test with unequal frequencies
    result = formula.build("0.7", "0.3")
    assert result == "0.7^2 + 2*0.7*0.3 + 0.3^2"
