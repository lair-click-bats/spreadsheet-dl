...
class TestCmdGenerate:
    ...
    def test_generate_with_template_success(self, tmp_path: Path) -> None:
        """Test generate with professional template."""
        args = argparse.Namespace(
            output=tmp_path,
            template="enterprise_budget",
            month=None,
            year=None,
            theme=None,
            yes=False,
        ...
        with patch("spreadsheet_dl.templates.professional.list_templates") as mock_list:
            mock_list.return_value = ["enterprise_budget", "cash_flow"]
            with patch(
                "spreadsheet_dl.templates.professional.get_template"
            ) as mock_get:
                mock_template_cls = Mock()
                mock_template_inst = Mock()
                mock_builder = Mock()
                mock_template_cls.return_value = mock_template_inst
                mock_template_inst.generate.return_value = mock_builder
                mock_get.return_value = mock_template_cls
    ...
    def test_generate_with_unknown_template(self, tmp_path: Path) -> None:
        ...
        with patch("spreadsheet_dl.templates.professional.list_templates") as mock_list:
            mock_list.return_value = ["enterprise_budget"]

            result = commands.cmd_generate(args)

            assert result == 1

    def test_generate_with_template_not_found(self, tmp_path: Path) -> None:
        """Test generate when template class is None."""
        args = argparse.Namespace(
            output=tmp_path,
            template="enterprise_budget",
            month=None,
    ...
    def test_generate_with_template_and_theme(self, tmp_path: Path) -> None:
        """Test generate with template and theme."""
        args = argparse.Namespace(
            output=tmp_path,
            template="enterprise_budget",
            month=None,
        ...
        with patch("spreadsheet_dl.templates.professional.list_templates") as mock_list:
            ...
            with patch(
                ...
                assert result == 0
                mock_template_cls.assert_called_once_with(theme="corporate")
