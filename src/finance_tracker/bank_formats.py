...
BUILTIN_FORMATS: dict[str, BankFormatDefinition] = {
    ...
    "chase_credit": BankFormatDefinition(
        ...
        header_patterns=["transaction date", "post date", "description", "category", "amount"],
    ),
    ...
    "bank_of_america_credit": BankFormatDefinition(
        ...
        reference_column="Reference Number",
        header_patterns=["posted date", "reference number", "payee", "address", "amount"],
    ),
    ...
    "capital_one_checking": BankFormatDefinition(
        ...
        description_column="Description",
        expense_is_negative=False,
        header_patterns=["transaction date", "transaction description", "debit", "credit"],
    ),
    "capital_one_credit": BankFormatDefinition(
        ...
        category_column="Category",
        expense_is_negative=False,
        header_patterns=["transaction date", "posted date", "card no", "description", "category", "debit", "credit"],
    ),
    ...
    "discover_credit": BankFormatDefinition(
        ...
        description_column="Description",
        category_column="Category",
        header_patterns=["trans. date", "post date", "description", "amount", "category"],
    ),
    ...
    "paypal": BankFormatDefinition(
        ...
        amount_column="Net",
        description_column="Name",
        header_patterns=["date", "time", "name", "type", "status", "currency", "gross", "fee", "net"],
    ),
    "venmo": BankFormatDefinition(
        ...
        header_patterns=["id", "datetime", "type", "status", "note", "from", "to", "amount"],
    ),
    ...
    "apple_card": BankFormatDefinition(
        ...
        category_column="Category",
        header_patterns=["transaction date", "clearing date", "description", "merchant", "category", "amount"],
    ),
    ...
    "mint": BankFormatDefinition(
        ...
        description_column="Description",
        category_column="Category",
        header_patterns=["date", "description", "original description", "amount", "transaction type", "category"],
    ),
...
class BankFormatRegistry:
    ...
    def __init__(
        ...
        if custom_dir:
            ...
            self._custom_dir = Path.home() / ".config" / "finance-tracker" / "bank-formats"
    ...
    def _score_format(
        ...
        if fmt.header_patterns:
            matches = sum(
                1 for pattern in fmt.header_patterns
...
class FormatBuilder:
    ...
    def from_csv_headers(
        ...
        for header in headers:
            ...
            if header_lower in ("description", "desc", "details", "payee", "name", "merchant"):
            ...
            if header_lower in ("reference", "ref", "check", "check #", "reference number"):
    ...
    def build(self, format_id: str) -> BankFormatDefinition:
        ...
        if not self._data["name"]:
            self._data["name"] = f"{self._data['institution']} - {self._data['format_type'].title()}"
