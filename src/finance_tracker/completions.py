...
COMMAND_STRUCTURE: dict[str, dict[str, Any]] = {
    "generate": {
        ...
        "options": {
            ...
            "--template": {
                "type": "choice",
                "choices": ["50_30_20", "family", "minimalist", "zero_based", "fire", "high_income"],
        ...
        "options": {
            ...
            "--category": {
                ...
                "choices": [
                    ...
                    "clothing", "personal_care", "education", "savings",
                    "debt_payment", "gifts", "subscriptions", "miscellaneous",
                ],
    ...
    "import": {
        ...
        "options": {
            "--bank": {
                "type": "choice",
                "choices": [
                    "chase", "chase_credit", "bank_of_america", "wells_fargo",
                    "capital_one", "discover", "amex", "usaa", "generic",
                ],
    ...
    "account": {
        ...
        "options": {
            "--type": {
                "type": "choice",
                "choices": ["checking", "savings", "credit_card", "investment", "loan", "cash", "other"],
    ...
    "goal": {
        ...
        "options": {
            "--category": {
                "type": "choice",
                "choices": ["savings", "emergency_fund", "vacation", "home_down_payment", "car_purchase", "education", "retirement"],
...
def generate_bash_completions() -> str:
    ...
    script = f'''# finance-tracker bash completion
# Generated by finance-tracker completions bash

_finance_tracker_completions()
{{
    ...
    for cmd, info in COMMAND_STRUCTURE.items():
        ...
        script += f'''        {cmd})
            if [[ $cword -eq 2 ]]; then
                ...
                _finance_tracker_complete_option "$cmd" "$prev" "$cur"
            fi
            ;;
'''
...
_finance_tracker_complete_option()
{
...
complete -F _finance_tracker_completions finance-tracker
'''
...
def generate_zsh_completions() -> str:
    ...
    script = '''#compdef finance-tracker
# finance-tracker zsh completion
# Generated by finance-tracker completions zsh
...
'''
    ...
    for cmd in COMMAND_STRUCTURE:
        script += f'''                {cmd})
                    _finance_tracker_{cmd.replace("-", "_")}
                    ;;
'''
...
'''
    ...
    for cmd, info in COMMAND_STRUCTURE.items():
        ...
        script += f'''_finance_tracker_{func_name}() {{
...
def generate_fish_completions() -> str:
    ...
    script = '''# finance-tracker fish completion
# Generated by finance-tracker completions fish

# Disable file completions by default
complete -c finance-tracker -f
...
'''
    ...
    for cmd, info in COMMAND_STRUCTURE.items():
        desc = info.get("description", cmd)
        script += f"complete -c finance-tracker -n '__fish_use_subcommand' -a '{cmd}' -d '{desc}'\n"
    ...
    for cmd, info in COMMAND_STRUCTURE.items():
        ...
        if "subcommands" in info:
            for sub, sub_info in info["subcommands"].items():
                desc = sub_info.get("description", sub)
                script += f"complete -c finance-tracker -n '__fish_seen_subcommand_from {cmd}' -a '{sub}' -d '{desc}'\n"
        ...
        if "options" in info:
            for opt, opt_info in info["options"].items():
                ...
                if opt_type == "flag":
                    script += f"complete -c finance-tracker -n '__fish_seen_subcommand_from {cmd}' -l '{opt_name}' -d '{desc}'\n"
                elif opt_type == "file":
                    script += f"complete -c finance-tracker -n '__fish_seen_subcommand_from {cmd}' -l '{opt_name}' -d '{desc}' -r -F\n"
                elif opt_type == "choice":
                    choices = opt_info.get("choices", [])
                    for choice in choices:
                        script += f"complete -c finance-tracker -n '__fish_seen_subcommand_from {cmd}; and __fish_prev_arg_in --{opt_name}' -a '{choice}'\n"
                    script += f"complete -c finance-tracker -n '__fish_seen_subcommand_from {cmd}' -l '{opt_name}' -d '{desc}' -r\n"
                else:
                    script += f"complete -c finance-tracker -n '__fish_seen_subcommand_from {cmd}' -l '{opt_name}' -d '{desc}' -r\n"
...
def _install_bash_completions() -> Path | None:
    ...
    system_paths = [
        Path("/etc/bash_completion.d/finance-tracker"),
        Path("/usr/share/bash-completion/completions/finance-tracker"),
    ]

    # User-local paths
    user_paths = [
        Path.home() / ".local/share/bash-completion/completions/finance-tracker",
        Path.home() / ".bash_completion.d/finance-tracker",
    ]
...
def _install_zsh_completions() -> Path | None:
    ...
    for path in fpath_dirs:
        completion_file = path / "_finance-tracker"
        try:
...
def _install_fish_completions() -> Path | None:
    ...
    paths = [
        Path.home() / ".config/fish/completions/finance-tracker.fish",
        Path("/usr/share/fish/vendor_completions.d/finance-tracker.fish"),
    ]
...
Option 1: Add to .bashrc (recommended)
    finance-tracker completions bash >> ~/.bash_completion
    source ~/.bashrc

Option 2: System-wide (requires sudo)
    sudo finance-tracker completions bash > /etc/bash_completion.d/finance-tracker

Option 3: User-local
    mkdir -p ~/.local/share/bash-completion/completions
    finance-tracker completions bash > ~/.local/share/bash-completion/completions/finance-tracker
"""
    elif shell == "zsh":
        return """
...
Option 1: Add to .zshrc (recommended)
    ...
    finance-tracker completions zsh > ~/.zsh/completions/_finance-tracker
...
Option 2: Oh My Zsh
    finance-tracker completions zsh > ~/.oh-my-zsh/completions/_finance-tracker
    omz reload
"""
    elif shell == "fish":
        return """
...
Option 1: User-local (recommended)
    ...
    finance-tracker completions fish > ~/.config/fish/completions/finance-tracker.fish

Option 2: System-wide (requires sudo)
    sudo finance-tracker completions fish > /usr/share/fish/vendor_completions.d/finance-tracker.fish
"""
    else:
