...
class AQICalculationFormula(BaseFormula):
    ...
    def build(self, *args: Any, **kwargs: Any) -> str:
        ...
        return (
            f"IF({concentration}<=12;{concentration}*4.166;"
...
class EmissionRateFormula(BaseFormula):
    ...
    def build(self, *args: Any, **kwargs: Any) -> str:
        ...
        if efficiency and str(efficiency) != "0":
            return f"{flow_rate}*{concentration}*(1-{efficiency})/1000000"
        else:
            return f"{flow_rate}*{concentration}/1000000"
...
class PollutionIndexFormula(BaseFormula):
    ...
    def build(self, *args: Any, **kwargs: Any) -> str:
        ...
        if o3_aqi and no2_aqi and str(o3_aqi) != "0" and str(no2_aqi) != "0":
            return f"MAX({pm25_aqi};{o3_aqi};{no2_aqi})+0.1*(({pm25_aqi}+{o3_aqi}+{no2_aqi})/3)"
        elif o3_aqi and str(o3_aqi) != "0":
            return f"MAX({pm25_aqi};{o3_aqi})+0.1*(({pm25_aqi}+{o3_aqi})/2)"
        else:
            return f"{pm25_aqi}"
