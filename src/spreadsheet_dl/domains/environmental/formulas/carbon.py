...
class EcologicalFootprintFormula(BaseFormula):
    ...
    def build(self, *args: Any, **kwargs: Any) -> str:
        ...
        if food and str(food) != "0" and housing and str(housing) != "0":
            return f"{carbon_gha}+{food}*0.8+{housing}*0.0001"
        elif food and str(food) != "0":
            return f"{carbon_gha}+{food}*0.8"
        elif housing and str(housing) != "0":
            return f"{carbon_gha}+{housing}*0.0001"
        else:
            return carbon_gha
...
class SustainabilityScoreFormula(BaseFormula):
    ...
    def build(self, *args: Any, **kwargs: Any) -> str:
        ...
        if (
            ...
            return f"{env_score}*0.4+{social_score}*0.3+{gov_score}*0.3"
        elif social_score and str(social_score) not in ("", "None"):
            return f"{env_score}*0.6+{social_score}*0.4"
        else:
            return f"{env_score}"
...
class EnvironmentalImpactScoreFormula(BaseFormula):
    ...
    def build(self, *args: Any, **kwargs: Any) -> str:
        ...
        return f"({magnitude}*{duration}*{reversibility}*{probability})/1.25"
