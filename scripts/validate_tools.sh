...
print_header "LATEX (chktex + latexindent)"

if check_tool "chktex"; then
    run_check "ChkTeX Linting" ".chktexrc" \
        "cd '$REPO_ROOT' && chktex -q '$TEST_DIR/test_latex.tex' 2>&1 | grep -v '^$' | wc -l | grep -q '^0$'"
else
    skip_check "ChkTeX" "chktex not installed"
fi

if check_tool "latexindent"; then
    run_check "latexindent Check" ".latexindent.yaml" \
        "cd '$REPO_ROOT' && latexindent -s -l '.latexindent.yaml' '$TEST_DIR/test_latex.tex' > /dev/null"
else
    skip_check "latexindent" "latexindent not installed"
fi
...
print_header "VHDL (ghdl + vsg)"

if check_tool "ghdl"; then
    run_check "GHDL Analysis" "vhdl_ls.toml" \
        "cd '$REPO_ROOT' && ghdl -a --std=08 '$TEST_DIR/test_vhdl.vhd'"
else
    skip_check "GHDL" "ghdl not installed"
fi

if check_tool "vsg"; then
    run_check "VHDL Style Guide" ".vsg.yaml" \
        "cd '$REPO_ROOT' && vsg -c '.vsg.yaml' '$TEST_DIR/test_vhdl.vhd'"
else
    skip_check "VSG" "vsg not installed"
fi
...
print_header "TCL (tclint)"

if check_tool "tclint"; then
    run_check "TclInt Linting" "tclint.toml" \
        "cd '$REPO_ROOT' && tclint '$TEST_DIR/test_tcl.tcl'"
else
    skip_check "TclInt" "tclint not installed"
fi
